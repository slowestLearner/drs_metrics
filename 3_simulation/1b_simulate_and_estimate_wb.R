...
gendata <- function(gm = 0.5, im = 0, b = 0, N = 1000, tt = 240, phi.mean, phi.sd) { ### b: drs
  ...
  # tmp <- readRDS('../tmp/simulation/inputs/cum_death_prob.RDS')
  # paste0(tmp[, round(1-cum_death_prob, 3)], collapse = ',')
  ...
  sr.c <- c(0.995, 0.964, 0.912, 0.863, 0.813, 0.771, 0.731, 0.688, 0.655, 0.63, 0.602, 0.572, 0.544, 0.519, 0.501, 0.489, 0.474, 0.465, 0.446, 0.435)
  ## yearly survival rate
  sr <- c(sr.c[1], sr.c[-1] / sr.c[-length(sr.c)])
...
simest.gm <- function(dat, rule, ni = 24) # rule = 0,1,2,3, 4 is the J-WB
{
  ...
  ## J-updated CW estimator
  dd_new <- copy(dd) %>% setDT()
  sizefe <- feols(size ~ lagsize + I(ret + Bret) + ps | fundid, cluster = c("fundid"), data = dd_new, notes = FALSE)
  fe_fund <- fixef(sizefe)$fundid
  dd_new$fe_fund <- rep(fe_fund, table(dd_new$fundid))

  # predicted component from the return-related parts
  dd_new$pred_perf <- sizefe$coefficients[2] * (dd_new$ret + dd_new$Bret) + sizefe$coefficients[3] * dd_new$ps
  dd_new <- merge(dd_new, dd_new[, .(obs = obs + 1, fundid, lag_pred_perf = pred_perf)], by = c("obs", "fundid"))

  dd_new <- dd_new[!is.na(lag_pred_perf)]
  cw_new <- lmer(ret ~ lagsize + fe_fund + lag_pred_perf + inisize + (1 | fundid), data = dd_new)

  ## output
  best <- c(
    # coef(ols)[2],
    coef(fe), rdout[1], fixef(re)[2], fixef(cw)[2], fixef(cw_new)[2]
    # , fixef(cw2)[2]
  )
  ...
  test.cw_new <- robust.se.remodel(dat = dd_new, model = cw_new, cluster = c("fundid"))
  # test.cw2 <- robust.se.remodel(dat = dd, model = cw2, cluster = c("fundid"))

  tv <- c(
    # test.ols[2, 3],
    fe$coeftable[, 3], rdout[2], test.re[2, 3], test.cw[2, 3], test.cw_new[2, 3]
    # , test.cw2[2, 3]
  )
...
for (this_spec_idx in specs$spec_idx) {
  ...
  sim_iteration <- function(i, gm_val, b_val) {
    tmp <- gendata(gm = gm_val, im = 0, b = b_val, N = 1000, tt = 240, phi.mean = 0.12, phi.sd = 0.03)
    dd <- tmp$data
