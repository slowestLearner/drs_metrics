# --- Let me allow for the possibility of longer history. I'm going to use my own probabilities
library(this.path)
...
gendata <- function(gm = 0.5, im = 0, b = 0, N = 1000, tt = 240, phi.mean, phi.sd) { ### b: drs
  ...
  q0 <- runif(N, log(20), log(20000)) # Initial fund size in logarithm
...
for (this_spec_idx in specs$spec_idx) {
  ...
  sim_iteration <- function(i, gm_val, b_val) {
    tmp <- gendata(gm = gm_val, im = 0, b = b_val, N = 1000, tt = 240, phi.mean = 0, phi.sd = 0.01)
    dd <- tmp$data
  ...
  to_dir <- paste0("tmp/sim_data_240_periods/nsim_", nsim, "_no_rho2_rho3/")
