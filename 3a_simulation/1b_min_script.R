# --- note --- changed initial size and also phi growth rate
library(this.path)
...
gendata <- function(gm = 0.5, im = 0, b = 0, N = 1000, tt = 200, phi.mean = 0, phi.sd = 0.01) { ### b: drs
...
for (this_spec_idx in specs$spec_idx) {
  ...
  sim_iteration <- function(i, gm_val, b_val) {
    tmp <- gendata(gm = gm_val, im = 0, b = b_val, N = 1000, tt = 120, phi.mean = 0, phi.sd = 0.01)
    dd <- tmp$data
  ...
  to_dir <- paste0("tmp/sim_1b/", nsim, "_nsim_phi_mean_0_T120_max_q0_100/")
